import { i18n, ajax } from '../../../../../frontend/express/public/javascripts/countly/vue/core.js';
import { countlyCommon } from '../../../../../frontend/express/public/javascripts/countly/countly.common.js';
import countlyGlobal from '../../../../../frontend/express/public/javascripts/countly/countly.global.js';
import { getPeriodUrlQueryParameter, notify } from '../../../../../frontend/express/public/javascripts/countly/countly.helpers.js';
...
var countlyDataManager = window.countlyDataManager || {};
    ...
    fetchLimits: function(period) {
        return ajax({
            ...
            data: {
                "app_id": countlyCommon.ACTIVE_APP_ID,
                "method": "get_events",
                "period": getPeriodUrlQueryParameter(period),
                "preventRequestAbort": true
            },
            dataType: "json",
        }, {"disableAutoCatch": true});
    ...
    var actions = {
        ...
        loadEventsData: function(context) {
            ...
            evenLoaderService().then(function(data) {
                ...
                if (window.countlyEventsOverview && window.countlyEventsOverview.service) {
                    window.countlyEventsOverview.service.fetchEvents().then(function(topEventData) {
        ...
        deleteEventGroups: function(context, events) {
            countlyDataManager.service.deleteEventGroups(events).then(function(data) {
                ...
                notify({message: i18n('data-manager.success.event-group-delete'), sticky: false, type: 'success'});
                return data;
            }).catch(function() {
                notify({message: i18n('data-manager.error.event-group-delete'), sticky: false, type: 'error'});
            });
        },
        ...
        saveEventGroups: function(context, data) {
            countlyDataManager.service.createEventGroups(data).then(function(res) {
                notify({message: i18n('data-manager.success.event-group-create'), sticky: false, type: 'success'});
                context.dispatch("loadEventGroups");
                return res;
            }).catch(function() {
                notify({message: i18n('data-manager.error.event-group-create'), sticky: false, type: 'error'});
            });
        },
        editEventGroups: function(context, data) {
            countlyDataManager.service.editEventGroups(data).then(function(res) {
                notify({message: i18n('data-manager.success.event-group-update'), sticky: false, type: 'success'});
                context.dispatch("loadEventGroups");
                return res;
            }).catch(function() {
                notify({message: i18n('data-manager.error.event-group-update'), sticky: false, type: 'error'});
            });
        },
        saveEvent: function(context, event) {
            countlyDataManager.service.saveEvent(event).then(function(err) {
                if (err !== 'Error') {
                    notify({message: i18n('data-manager.success.event-create'), sticky: false, type: 'success'});
                    context.dispatch('loadEventsData');
                    context.dispatch('loadSegmentsMap');
                }
                else {
                    notify({message: i18n('data-manager.error.event-create'), sticky: false, type: 'error'});
                    return err;
                }
            }).catch(function(err) {
                notify({message: i18n('data-manager.error.event-create'), sticky: false, type: 'error'});
        ...
        editEvent: function(context, event) {
            ...
            countlyDataManager.service.editEvent(eventMap, omittedSegments).then(function(err) {
                if (isDrill && typeof countlyDataManager.service.editEventMeta === 'function') {
                    countlyDataManager.service.editEventMeta(eventMeta).then(function(errMeta) {
                        if (err === 'Error' || errMeta === "Error") {
                            notify({message: i18n('data-manager.error.event-update'), sticky: false, type: 'error'});
                            return 'Error';
                        }
                        notify({message: i18n('data-manager.success.event-update'), sticky: false, type: 'success'});
                        context.dispatch('loadEventsData');
        ...
        changeVisibility: function(context, data) {
            ...
            countlyDataManager.service.changeVisibility(data.events, visibility, isDrill).then(function(res) {
                if (res === 'EVENT_STATUS_UNPLANNED') {
                    ...
                    notify({message: i18n('data-manager.success.visibility'), sticky: false, type: 'success'});
                }
                context.dispatch('loadEventsData');
                context.dispatch('loadSegmentsMap');
            }).catch(function(e) {
        ...
        deleteEvents: function(context, events) {
            countlyDataManager.service.deleteEvents(events).then(function(res) {
                if (res === 'Error') {
                    notify({message: i18n('data-manager.error.event-delete'), sticky: false, type: 'error'});
                    return 'Error';
                }
                context.dispatch('loadEventsData');
                context.dispatch('loadSegmentsMap');
                notify({message: i18n('data-manager.success.event-delete'), sticky: false, type: 'success'});
            }).catch(function(e) {
                if (e.status === 504) {
                    notify({message: i18n('data-manager.error.event-delete-timeout'), sticky: false, type: 'warning'});
                }
                else {
                    notify({message: i18n('data-manager.error.event-delete'), sticky: false, type: 'error'});
                }
        ...
        saveCategories: function(context, categories) {
            countlyDataManager.service.createCategory(categories).then(function(data) {
                notify({message: i18n('data-manager.success.category-create'), sticky: false, type: 'success'});
                context.dispatch('loadEventsData');
                context.dispatch('loadSegmentsMap');
                context.dispatch('loadCategories');
                return data;
            }).catch(function() {
                notify({message: i18n('data-manager.error.category-create'), sticky: false, type: 'error'});
            });
        },
        editCategories: function(context, categories) {
            return countlyDataManager.service.editCategories(categories).then(function(data) {
                notify({message: i18n('data-manager.success.category-update'), sticky: false, type: 'success'});
                context.dispatch('loadEventsData');
                context.dispatch('loadSegmentsMap');
                context.dispatch('loadCategories');
                return data;
            }).catch(function() {
                notify({message: i18n('data-manager.error.category-update'), sticky: false, type: 'error'});
            });
        },
        deleteCategories: function(context, categories) {
            countlyDataManager.service.deleteCategories(categories).then(function(data) {
                notify({message: i18n('data-manager.success.category-delete'), sticky: false, type: 'success'});
                context.dispatch('loadCategories');
                return data;
            }).catch(function() {
                notify({message: i18n('data-manager.error.category-delete'), sticky: false, type: 'error'});
            });
        },
        changeCategory: function(context, data) {
            countlyDataManager.service.changeCategory(data.events, data.category).then(function(res) {
                context.dispatch('loadEventsData');
                context.dispatch('loadSegmentsMap');
                notify({message: i18n('data-manager.success.category-change'), sticky: false, type: 'success'});
                return res;
            }).catch(function() {
                notify({message: i18n('data-manager.error.category-change'), sticky: false, type: 'error'});
            });
        },
    };

    return Module("countlyDataManager", {
        state: getEmptyState,
...
window.countlyDataManager = countlyDataManager;

export default countlyDataManager;
