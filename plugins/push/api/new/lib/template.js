 ...
 * @typedef {User|{ [key: string]: any; }} TemplateContext
 * @typedef {(platform: PlatformKey, context: TemplateContext) => PlatformMessagePayload} MessageTemplateFunction
 */
 ...
 * @returns {MessageTemplateFunction} Template function to compile message content.
 */
function createTemplate(messageDoc) {
     ...
     * @param {TemplateContext} context - app user properties with push token populated and custom variables
     * @returns {PlatformMessagePayload} compiled message content
     */
    return function(platform, context) {
 ...
 * @param {TemplateContext} context - Data object containing user properties and other variables to be used in the template.
 * @returns {Content} Compiled content with personalizations applied.
 */
function compilePersonalizableContent(contentPers, context) {
