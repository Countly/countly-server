...
class Note {
    ...
    insert (db) {
        return new Promise((resolve, reject) => {
            db.collection('messages').insertOne(this.toJSON(), (err, res) => {
                if (err || !res || !res.insertedCount) { reject(err || 'WTF'); } 
                else { resolve(); }
            });
        });
    }
    ...
    update (db, update) {
        return new Promise((resolve, reject) => {
            db.collection('messages').updateOne({_id: typeof this._id === 'string' ? db.ObjectID(this._id) : this._id}, update, (err, res) => {
                if (err || !res) { reject(err || 'WTF'); } 
                else if (!res.matchedCount) { reject('Not found'); } 
    ...
    personalize(str, pers, user) {
        ...
        var ret = str;
        Object.keys(pers).sort(function(a, b){ return parseInt(b) - parseInt(a); }).forEach(function(k){
            var p = pers[k],
    ...
    compile (platform, msg) {
        if (this.platforms.indexOf(platform) === -1) { return null; }
        ...
        if (platform === Platform.IOS) {
            ...
            if (data) {
                for (let k in data) { compiled[k] = data[k]; }
            }
