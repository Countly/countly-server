...
PushlyWorker.prototype.connect = function(message) {
	...
	if (!connection) {
		...
		connection.on(SP.CLOSED, function(conn){
			...
			debug('Cluster is closed, removing cluster %j from connections: %d / %j', conn.credentials, this.connections.length, this.connections);
			this.connections.remove(conn.credentials.id);
			debug('Cluster is closed, removed cluster %j from connections: %d / %j', conn.credentials, this.connections.length, this.connections);
...
PushlyWorker.prototype.updateMessage = function(message, immediate, error) {
	...
	if (immediate || !this.messagesStatusTimes || !this.messagesStatusTimes[message.id] || this.messagesStatusTimes[message.id] < (Date.now() - this.options.statusUpdatePeriod)) {
		...
		debug('%j: Updated message in process %d with master notification: %j, %j, %j', new Date().toString(), process.pid, message, immediate, error);
	} else {
		debug('%j: Updated message in process %d: %j, %j, %j (immediate %d, statusTimes %d)', new Date().toString(), process.pid, message, immediate, error, immediate, this.messagesStatusTimes[message.id] - (Date.now() - this.options.statusUpdatePeriod));
	}
