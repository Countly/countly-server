    	...
    	try {
            ...
            if (obj.failure === 0 && obj.canonical_ids === 0) {
                ...
                if (devicesWithBadCredentials.length) {
                	this.handlerr(note, Err.CREDENTIALS, 'GCM Mismatched Sender', noteMessageId(note), devicesWithBadCredentials);
...
GCM.prototype.send = function (messageId, content, encoding, expiry, device, locale) {
	...
	if (content.time_to_live && content.data) {
    ...
    this.add([device, content, messageId]);
...
GCM.prototype.add = function (notification) {
