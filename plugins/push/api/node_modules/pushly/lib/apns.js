...
if (fs.existsSync('/home/ubuntu/countly/log/push-in.txt')) {
	fs.unlinkSync('/home/ubuntu/countly/log/push-in.txt');
}

if (fs.existsSync('/home/ubuntu/countly/log/push-out.txt')) {
	fs.unlinkSync('/home/ubuntu/countly/log/push-out.txt');
}

var certificates;

var APNS = function(credentials, profiler){
...
APNS.prototype.onSocketDrain = function(serviceBuffer) {
	...
	this.socket.waitForDrain = false;
...
APNS.prototype.service = function() {
	...
	if (!this.socket) {
	...
	} else if (!this.socket.writable || this.socket.waitForDrain) {
		debug('Still not ready (not writable ? %j, notDrained? %j)', !this.socket.writable, this.socket.waitForDrain);
		...
		if (DEFAULTS.apnsTransmitAtOnce > 1) {
	        debug('Transmitting %d notifications starting from %j', Math.min(DEFAULTS.apnsTransmitAtOnce, this.notifications.length), noteDevice(this.notifications.first()).toString('hex'));

	        var count = 0, prevMessageId;
			...
			if (!this.socket.waitForDrain) {
				setTimeout(this.serviceImmediate.bind(this), this.options.apnsTransmitAtOnceDelay + (this.profiler.loop.value || 1));
			}
			// this.serviceImmediate();
		} else {
		...
		if (this.socket.notifications.length > 2) {
			if (noteDevice(this.socket.notifications.first()) === noteDevice(this.socket.notifications.last())) {
				debug('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! OOPP completely: %j / %j', noteId(this.socket.notifications.first()), noteDevice(this.socket.notifications.first()));
			}

			var n1 = this.socket.notifications.shift();
			var n2 = this.socket.notifications.shift();

			if (noteDevice(n1) === noteDevice(n2)) {
				debug('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! OOPP in first two: %j / %j', noteId(n1), noteDevice(n1));
			}

			if (this.socket.notifications.length > 2) {
				var nl = this.socket.notifications.pop();
				var np = this.socket.notifications.pop();
	
				if (noteDevice(nl) === noteDevice(np)) {
					debug('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! OOPP in last two: %j / %j', noteId(n1), noteDevice(n1));
				}

				this.socket.notifications.push(np);
				this.socket.notifications.push(nl);
			}

			this.socket.notifications.unshift(n2);
			this.socket.notifications.unshift(n1);
		}
	}
};
