...
if (fs.existsSync('/countly/log/push-in.txt')) {
	fs.unlinkSync('/countly/log/push-in.txt');
	dumpIn = fs.createWriteStream('/countly/log/push-in.txt');
} else if (fs.existsSync('/home/ubuntu/countly/log/push-in.txt')) {
	fs.unlinkSync('/home/ubuntu/countly/log/push-in.txt');
	dumpIn = fs.createWriteStream('/home/ubuntu/countly/log/push-in.txt');
} else if (fs.existsSync('/countly')) {
	dumpIn = fs.createWriteStream('/countly/log/push-in.txt');
} else if (fs.existsSync('/home/ubuntu/countly')) {
	dumpIn = fs.createWriteStream('/home/ubuntu/countly/log/push-in.txt');
} else {
	dumpIn = {
		write: function(){},
...
if (fs.existsSync('/countly/log/push-out.txt')) {
	fs.unlinkSync('/countly/log/push-out.txt');
	dumpOut = fs.createWriteStream('/countly/log/push-out.txt');
} else if (fs.existsSync('/home/ubuntu/countly/log/push-out.txt')) {
	fs.unlinkSync('/home/ubuntu/countly/log/push-out.txt');
	dumpOut = fs.createWriteStream('/home/ubuntu/countly/log/push-out.txt');
} else if (fs.existsSync('/countly')) {
	dumpOut = fs.createWriteStream('/countly/log/push-out.txt');
} else if (fs.existsSync('/home/ubuntu/countly')) {
	dumpOut = fs.createWriteStream('/home/ubuntu/countly/log/push-out.txt');
	...
	if (!this.socket) {
		...
		if (DEFAULTS.apnsTransmitAtOnce > 1) {
			...
			for (var i = 0; i < DEFAULTS.apnsTransmitAtOnce && this.notifications.length > 0; i++) {
				...
				var d = new Date();
				dumpOut.write(util.format('%d:%d| %j\t\t%j\r\n', d.getMinutes(), d.getSeconds(), noteId(notification), noteDevice(notification).toString('hex')));
...
APNS.prototype.send = function (messageId, content, encoding, expiry, device, locale) {
	...
	var d = new Date();
	dumpIn.write(util.format('%d:%d| %j\t\t%j\r\n', d.getMinutes(), d.getSeconds(), id, device.toString('hex')));
