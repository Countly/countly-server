...
var certificates, APNSCount = 0;

var APNS = function(credentials){
	debug = constants.debug('APNS' + APNSCount++);
...
APNS.prototype.onSocketData = function (data) {
	if (data[0] === 8) {
		...
		debug('Scrolled %d notifications, %d left', que.length, this.socket.notifications.length);
		...
		debug('Putting back %d notifications because of error %d on %d notification', this.socket.notifications.length, code, identifier);
		...
		if (DEFAULTS.apnsTransmitAtOnce > 1) {
			...
			for (var i = 0; i < DEFAULTS.apnsTransmitAtOnce && this.notifications.length > 0; i++) {
				...
				if (!this.socket.write(noteData(notification))) {
	        		debug('Failed to write to the socket, skipping 1 (%d / %d)', this.socket.notifications.length, this.notifications.length);
					break;
				}
