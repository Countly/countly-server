...
describe('Push', function(){
    ...
    function createUser(id, callback){
        request.get(`/i?device_id=${id}&app_key=${APP_KEY}&begin_session=1&tz=${users[id].tz}`).expect(200).end(function(err, res){
            if (err) {
                return callback(err);
            }
            ...
            request.get(`/i?device_id=${id}&app_key=${APP_KEY}&token_session=1&${users[id].tkip ? 'ios_token=' + users[id].tkip : 'android_token=' + users[id].tkap}&test_mode=0&locale=${users[id].locale}`).expect(200).end(function(err, res){
                if (err) {
                    return callback(err);
                }
    ...
    describe('streaming', () => {
        ...
        const db = require('../pluginManager.js').singleDefaultConnection(),
               ...
               credentialsIOS = new Credentials('cidIOS'),
        ...
        describe('(timezoned)', () => {
            ...
            it('should load with tz correctly', done => {
                ...
                streamer.load(db, null, null, 100, min.getTime(), max.getTime()).then(usrs => {
                    usrs.length.should.equal(0); // nobody's after GMT+3
                    ...
                    streamer.load(db, null, null, 100, min.getTime(), max.getTime()).then(usrs => {
                        usrs.length.should.equal(3); // ru, tk, unknown

                        streamer.unload(db, usrs.map(u => u._id)).then(ok => {
                            ...
                            streamer.load(db, null, null, 100, min, max).then(usrs => {
                                usrs.length.should.equal(1); // lv
                                        ...
                                        streamer.unload(db, max).then(ok => {
                                            ...
                                            streamer.load(db, null, null, 100, min, max).then(usrs => {
                                                ...
                                                streamer.unload(db, max).then(ok => {
                                                    ...
                                                    streamer.load(db, null, null, 100, min, max).then(usrs => {
                                                        usrs.length.should.equal(1); // us

                                                        streamer.unload(db, usrs.map(u => u._id)).then(ok => {
                                                            should.not.exist(nex); // nobody left
                                                            done();
                                                        }, done);
                                                    }, done);
                                                }, done);
            ...
            it('should process simple auto messages correctly', done => {
                ...
                db.collection('apps').findOne({key: APP_KEY}, (err, app) => {
                    ...
                    divider.store(db, app, [findUser('ios_ru').uid], now).then(() => {
                        db.collection(streamerIOS.collection()).find().toArray((err, users) => {
                            should.not.exist(err);
                            users.length.should.equal(1);
                            users[0].tkip.should.equal('ios_ru');
                            users[0].da.should.equal(momenttz.tz('2017-12-01 13:00', 'Europe/Moscow').toDate().getTime());
                            ...
                            divider.store(db, app, [findUser('ios_ru').uid], now).then(() => {
                                db.collection(streamerIOS.collection()).find().toArray((err, users) => {
                                    ...
                                    users.length.should.equal(1);
                                    users[0].tkip.should.equal('ios_ru');
                                    users[0].da.should.equal(momenttz.tz('2017-12-02 13:00', 'Europe/Moscow').toDate().getTime());
