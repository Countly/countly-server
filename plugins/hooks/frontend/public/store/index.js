...
import { validateUpdate, validateDelete } from '../../../../../frontend/express/public/javascripts/countly/countly.auth.js';
    ...
    var actions = {
            ...
            return jQuery.ajax({
                ...
                success: function(data) {
                    if (data.hooksList && data.hooksList.length === 1) {
                        ...
                        record._canUpdate = validateUpdate(FEATURE_NAME, countlyGlobal.member, record.apps[0]),
                        record._canDelete = validateDelete(FEATURE_NAME, countlyGlobal.member, record.apps[0]),
        ...
        saveHook: function(context, record) {
            ...
            return jQuery.ajax({
                ...
                error: function() {
                    notify({type: "error", message: jQuery.i18n.map["hooks.trigger-save-failed"]});
                }
            });
        },
        deleteHook: function(context, id) {
            return jQuery.ajax({
                ...
                success: function() {
                    context.dispatch("countlyHooks/table/fetchAll", null, {root: true});
                    app.navigate("/manage/hooks", true);
                }
            });
        },
                ...
                return jQuery.ajax({
                    ...
                    for (var i = 0; i < hookList.length; i++) {
                        ...
                        tableData.push({
                            ...
                            _canUpdate: validateUpdate(FEATURE_NAME, countlyGlobal.member, hookList[i].apps[0]),
                            _canDelete: validateDelete(FEATURE_NAME, countlyGlobal.member, hookList[i].apps[0]),
                        });
                    }
...
window.hooksPlugin = hooksPlugin;
