/*global
  _, jQuery, $, countlySegmentation, countlyEvent,countlyGlobal, countlyCommon,moment, CountlyHelpers, app,
 */
...
(function(hooksPlugin, jQuery) {
    var _hookTriggers = {
        "IncomingDataTrigger": {
            ...
            showIntro: function() {
                $(".trigger-intro").css("display", "block");
                var text = jQuery.i18n.map["hooks.intro-incoming-data"];
                $(".trigger-intro-content").html(text);
            },
        ...
        "InternalEventTrigger": {
            ...
            init: function() {
                ...
                $("#single-hook-trigger-internal-event-dropdown")
                    .off("cly-select-change").on("cly-select-change", function(e, selected) {
                        $(".internal-event-data-description").text(selected);
            ...
            renderConfig: function(trigger) {
                ...
                case "/cohort/exit":
                    setTimeout(function() {
                        self.loadCohortsData(configuration);
                    }, 200);
                    break;
                case "/hooks/trigger":
                    setTimeout(function() {
                        self.loadHooksData(configuration);
                    }, 200);
                    break;
                }
            },
                    ...
                    $.ajax({
                        ...
                        success: function(cohorts) {
                            ...
                            cohortItems.forEach(function(i) {
                                if (self.tmpCohortConfig && i.value === self.tmpCohortConfig.cohortID) {
            ...
            showIntro: function(event) {
                $(".trigger-intro").css("display", "block");
                var text = jQuery.i18n.map["hooks.intro" + event.split('/').join('-')];
                $(".trigger-intro-content").html(text);
            },
            loadEventView: function(event) {
                ...
                case "/cohort/enter":
                case "/cohort/exit":
                    html = $("#template-hook-trigger-cohort-view").html();
        ...
        "APIEndPointTrigger": {
            ...
            init: function() {
                 ...
                 * @return {string} - generated uuid4 value
                 */
                function uuidv4() {
            ...
            renderIntro: function() {
                ...
                $(".trigger-intro").css("display", "block");
                var html = jQuery.i18n.prop("hooks.trigger-api-endpoint-intro-content", url)
                + "<span style='color:#23a321;margin-left: 8px; cursor:pointer;' class='hook-trigger-url'>" + jQuery.i18n.map["hooks.trigger-copy-url"] + "</span><textarea id='url-box' style='position:fixed; z-index:-10;top:10px;'/>";
                $(".trigger-intro-content").html(html);
                $(".hook-trigger-url").off("click").on("click", function() {
        ...
        "ScheduledTrigger": {
            name: jQuery.i18n.map["hooks.ScheduledTrigger"],
            init: function() {
                ...
                for (var key in countlyGlobal.timezones) {
                    zones.push({value:key.toLowerCase(), name:countlyGlobal.timezones[key].n});
                }
                $("#hook-trigger-schedule-timezone-drop").clySelectSetItems(zones);
                $("#hook-trigger-schedule-timezone-drop").off("cly-select-change").on("cly-select-change", function(e, selected) {
                    console.log(e, selected, countlyGlobal.timezones[selected.toUpperCase()]);
                    var c = countlyGlobal.timezones[selected.toUpperCase()];
                    var zones2 = [];
                    c.z.forEach(function(z) {
                        for(var t in z) {
                            zones2.push({value:z[t], name:t})
                        }
                    });
                    $("#hook-trigger-schedule-timezone-drop2").clySelectSetItems(zones2);
                });

                this.cron = "59 23 * * *";
                var self = this;
                this.cronUI = $('#cron-selector').cron({
                    initial: this.cron,
                    onChange: function() {
                        console.log($(this).cron("value"));
                        self.cron = $(this).cron("value");
                    },
                    minuteOpts: {
            ...
            renderConfig: function(trigger) {
                var configuration = trigger.configuration;
                $("#hook-trigger-schedule-timezone-drop").clySelectSetSelection(configuration.timezone1,countlyGlobal.timezones[configuration.timezone1.toUpperCase()].n);
                
                setTimeout(function() {
                    countlyGlobal.timezones[configuration.timezone1.toUpperCase()].z.forEach(function(zone){
                    for(var t in zone) {
                        if (zone[t] === configuration.timezone2) {
                            $("#hook-trigger-schedule-timezone-drop2").clySelectSetSelection(configuration.timezone2, zone[t]);
                        }
                    }})
                },200);

                this.cron = configuration.cron;
                this.cronUI.cron("value", this.cron);
            },
            getValidConfig: function() {
                var timezone1 = $("#hook-trigger-schedule-timezone-drop").clySelectGetSelection();
                var timezone2 = $("#hook-trigger-schedule-timezone-drop2").clySelectGetSelection();
                console.log({timezone1, timezone2, cron: this.cron });

                

                return {timezone1, timezone2, cron: this.cron };
            },
            renderIntro: function() {
                var url = window.location.protocol + "//" + window.location.host + "/o/hooks/" + $("#api-endpoint-trigger-uri").val();
                $(".trigger-intro").css("display", "block");
