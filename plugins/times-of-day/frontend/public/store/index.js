...
import { getEvents } from '../../../../../frontend/express/public/javascripts/countly/countly.event.js';
...
var service = {
    ...
    findEventKeyByName: function(name) {
        return getEvents().find(function(item) {
            return item.name === name;
        });
    },
    getEventOptions: function() {
        return getEvents().map(function(event) {
    ...
    getDateBucketsList: function() {
        var self = this;
        return [
            {label: i18n('times-of-day.previous-month'), value: self.DateBucketEnum.PREV_MONTH},
            {label: i18n('times-of-day.this-month'), value: self.DateBucketEnum.THIS_MONTH},
            {label: i18n('times-of-day.last-three-months'), value: self.DateBucketEnum.LAST_THREE_MONTHS},
            {label: i18n('times-of-day.all-time'), value: self.DateBucketEnum.ALL_TIME}
        ];
    },
...
var getInitialState = function() {
    return {
        ...
        series: [],
        filters: {
            dateBucketValue: service.DateBucketEnum.ALL_TIME,
...
var countlyTimesOfDayActions = {
    fetchAll: function(context, useLoader) {
        ...
        service.fetchAll(context.state.filters)
...
var countlyTimesOfDay = {
    service: service,
    getVuexModule: function() {
        return Module("countlyTimesOfDay", {
...
export default countlyTimesOfDay;
export { service };
