window.starView = countlyView.extend({
    ...
    loadRatingData: function() {
        this.templateData['rating_options'] = [{"val":1,"title":"Very dissatisfied"},{"val":2,"title":"Somewhat dissatisfied"},{"val":3,"title":"Neither satisfied nor dissatisfied"},{"val":4,"title":"Somewhat satisfied"},{"val":5,"title":"Very satisfied"}]
        var index = 0;
        ...
        if (isRefresh) {
            ...
            var columnsDefine = [
                ...
                },
                {
                    "mData":function (row) {
                        var target_pages = "";
                        eval(row.target_pages).forEach(function(page) { target_pages += "\n" + page;})
                ...
                },
                {
                    "mData":function (row) {
                        ...
                        if (td.length == 2) {
                            return td[0].replace(/\b\w/g, l => l.toUpperCase()) + " and " + td[1];
                        } else if(td.length == 3) {
            ...
            columnsDefine.push({
                "mData": function (row) {
                    if (!(countlyGlobal.member.admin_of && (countlyGlobal.member.admin_of.indexOf(countlyCommon.ACTIVE_APP_ID) !== -1)) && !(countlyGlobal.member.global_admin)) {
                        ...
                        "<div data-clipboard-text='"+row._id+"' class='copy-widget-id item'" + " data-id='" + row._id + "'" + ">Copy ID</div></div>" +
                        "</div>";
                    }
                },
                "bSortable": false,
    ...
    renderFeedbackDrawer: function() {
        ...
        if (this.step == 3) {
            ...
            $('#completed-1').css({"display":"inline-block"});
            $('#completed-2').css({"display":"inline-block"});
            $('#not-completed-1').css({"display":"none"});
            $('#not-completed-2').css({"display":"none"});
            $('.feedback-preview-footer').css({"display":"block"});
            $('.feedback-preview-title').text(jQuery.i18n.map['feedback.preview']);
        }
        else if (this.step == 2) {
            tippy('.show-tooltip',{'theme':'honeybee'});
            $('#countly-feedback-next-step').text(jQuery.i18n.map['feedback.next-step']);
            $('#countly-feedback-back-step').css({"display":"block"});   
            $('#completed-1').css({"display":"inline-block"});
            $('#not-completed-1').css({"display":"none"});
            $('#completed-2').css({"display":"none"});
            $('#not-completed-2').css({"display":"inline-block"});
            $('#feedback-preview-step-2').css({"display":"block"});
            ...
            $('#completed-1').css({"display":"none"});
            $('#not-completed-1').css({"display":"inline-block"});
            $('#completed-2').css({"display":"none"});
            $('#not-completed-2').css({"display":"inline-block"});
            $('#feedback-preview-step-1').css({"display":"block"});
    ...
    renderCommon: function (isRefresh) {
        ...
        if (!isRefresh) {
            ...
            $('.colorpicker').colorpicker({
                ...
                select: function(event, color) {
                    ...
                    if (self.feedbackWidget.trigger_bg_color.length > 6) {
                        $("#feedback_color_preview_1").css({"background-color":self.feedbackWidget.trigger_bg_color});
                    ...
                    if (self.feedbackWidget.trigger_font_color.length > 6) {
                        $("#feedback_color_preview_2").css({"background-color":self.feedbackWidget.trigger_font_color});
                    ...
                    self.updateConfig(id, {hex:color['hex'],alpha:""+a,rgba:rgba});
            ...
            if (countlyGlobal.member.global_admin) {
                $('#create-feedback-widget-button').css({"display":"block"});
                $('.options-item').css({"display":"block"});
                ...
                $('#create-feedback-widget-button').css({"display":"none"});
            }
            ...
            $('#thanks-modal').css({"border-left":"1px solid #2eb52b"});
                ...
                starRatingPlugin.requestSingleWidget($(this).data('id'), function(widget) {
                    ...
                    starRatingPlugin.editFeedbackWidget(self.feedbackWidget, function(result, status) {
                        if (status == 200) {
                            ...
                            CountlyHelpers.notify({type:'green', delay:1000, title:'Successfully updated', message:'Feedback widget '+result+' successfully.'});
                        } else {
                            CountlyHelpers.notify({type:'red', delay:1000, message:'Feedback widget couldn\'t updated.'});
                        }
                    })
                });
            })
            ...
            $("body").on("click", ".copy-widget-id", function () {
                $('.edit-menu').css({"display":"none"});
                CountlyHelpers.notify({type:'green', title:'Success', delay:1000, message:'Widget id copied to clipboard!'});
            });
            ...
            $("#create-feedback-widget-button").on("click", function () {
                ...
                // TODO: localize
                $('#feedback-drawer-title').html('Add new widget');
            ...
            $("body").on("click", ".edit-widget", function () {
                ...
                starRatingPlugin.requestSingleWidget($(this).data('id'), function(widget) {
                    ...
                    if (self.feedbackWidget.trigger_bg_color.length > 6) {
                        $('#feedback-sticker-on-window').css({"background-color":self.feedbackWidget.trigger_bg_color});    
                    } else {
                        $('#feedback-sticker-on-window').css({"background-color":'#'+self.feedbackWidget.trigger_bg_color});    
                    }

                    if (self.feedbackWidget.trigger_font_color.length > 6) {
                        $('#feedback-sticker-on-window').css({"color":self.feedbackWidget.trigger_font_color});    
                    } else {
                        $('#feedback-sticker-on-window').css({"color":'#'+self.feedbackWidget.trigger_font_color});    
                    }
            ...
            $('#feedback-popup-email-text').on('keyup', function() {
                self.feedbackModalToggle('popup');
                $('#counter-for-'+$(this).attr('id')).css({"display":"block"});
                if ($(this).val().length > 35) {
            ...
            $('.text-input-in-counter').on('focusout', function() {
                $('#counter-for-' + $(this).attr('id')).css({"display":"none"});
            })
