...
(function() {
    ...
    */
    ...
    const getFeedbackCollectionList = () => {
        return new Promise((resolve, reject) => {
            common.db.collections(function(err, collectionList) {
                if (!err) {
    ...
    */
    const extractWidgetIds = (collectionName) => {
        return new Promise((resolve, reject) => {
            let widgets = [];
            common.db.collection(collectionName).find({}).toArray(function(err, feedbacks) {
                if (!err) {
     ...
     */
    const fixWidgetDisappear = async(ob) => {
        try {
            ...
            asyncForeach(feedbackCollections, async function(collection, done) {
                ...
                asyncForeach(widgets, async function(widget, _done) {
                    ...
                    common.returnMessage(ob.params, 201, "Recovered related widgets from feedback data.");
                });
            });
        }
        catch (e) {
            common.returnMessage(ob.params, 500, e);
        }
    };

    plugins.register("/o/fix-widget-disappear", function(ob) {
        fixWidgetDisappear(ob);
        return true;
    });
}(exported));
