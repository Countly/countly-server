...
(function (plugin) {
    ...
    */
    ...
    plugins.register("/i/device_id", function(ob){
        ...
        if(oldUid != newUid){
            common.db.collection("feedback" +  appId).update({uid:oldUid}, {'$set': {uid:newUid}}, {multi:true}, function(err, res){});
        }
    });
    ...
    plugins.register("/i/app_users/delete", function(ob){
        ...
        var uids = ob.uids;
        if(uids && uids.length){
            common.db.collection("feedback" +  appId).remove({uid:{$in:uids}}, function(err) {});
        }
    });
