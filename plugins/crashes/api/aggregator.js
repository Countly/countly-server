...
const log = require('../../../api/utils/log.js')('crashes:aggregator'); //Please add log
                    ...
                    if (currEvent.e === '[CLY]_crash' && 'a' in currEvent) {
                        common.readBatcher.getOne('apps', common.db.ObjectID(currEvent.a), async(err, app) => {
                            ...
                            if (app && '_id' in app) {
                                ...
                                const platform = currEvent.up?.p || currEvent.sg?.os;
                                const version = currEvent.up?.av || currEvent.sg?.app_version?.replace(/\./g, ':');
                                ...
                                groupInsert.startTs = currEvent.ts;
                                ...
                                for (let i = 0; i < props.length; i++) {
                                    if (ranges.indexOf(props[i]) !== -1) {
                                        ...
                                        if (typeof currEvent.sg[props[i]] !== 'undefined') {
                                            let safeKey = (currEvent.sg[props[i]] + '').replace(/^\$/, '').replace(/\./g, ':');
                                            if (bools[props[i]]) {
                                                if (currEvent.sg[props[i]] + '' === 'false') {
                                                    safeKey = 'no';
                                                }
                                                else {
                                                    safeKey = 'yes';
                        ...
                        common.readBatcher.getOne('apps', common.db.ObjectID(currEvent.a), async(err, app) => {
                            ...
                            if (app && '_id' in app) {
                                ...
                                if (currEvent.up.ls) {
                                    ...
                                    const platform = currEvent.up?.p || currEvent.sg?.os;
                                    const version = currEvent.up?.av || currEvent.sg?.app_version?.replace(/\./g, ':');
                                    var ts0 = currEvent.sg.prev_start || 0;

                                    if (fatalMetrics.length) {
                                        recordCustomMetric(params, 'crashdata', params.app_id, fatalMetrics, 1, null, ['crauf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `${platform}**${version}**${params.app_id}`, fatalMetrics, 1, null, ['crauf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `${platform}**any**${params.app_id}`, fatalMetrics, 1, null, ['crauf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `any**${version}**${params.app_id}`, fatalMetrics, 1, null, ['crauf'], ts0, token, localWriteBatcher);
                                    }
                                    ...
                                    if (nonFatalMetrics.length) {
                                        recordCustomMetric(params, 'crashdata', params.app_id, nonFatalMetrics, 1, null, ['craunf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `${platform}**${version}**${params.app_id}`, nonFatalMetrics, 1, null, ['craunf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `${platform}**any**${params.app_id}`, nonFatalMetrics, 1, null, ['craunf'], ts0, token, localWriteBatcher);
                                        recordCustomMetric(params, 'crashdata', `any**${version}**${params.app_id}`, nonFatalMetrics, 1, null, ['craunf'], ts0, token, localWriteBatcher);
                                    }
                                }
                            }
                        });
