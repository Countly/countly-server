...
(function() {
    ...
    var exportOrGroup = function(rows) {
        return (rows || []).reduce(function(query, row) {
            ...
            if (indexedProps.includes(row.property.id)) {
                (Array.isArray(row.value.data) ? row.value.data : [row.value.data]).forEach(function(v) {
                    subquery[row.property.id + "." + v] = {$exists: !negateValue};
                });
            }
            else {
                var op = {
                ...
                if (typeof op === "string") {
                    ...
                    if (["$in", "$nin"].includes(op) && !Array.isArray(row.value.data)) {
            ...
            Object.keys(subquery).forEach(function(propId) {
                ...
                Object.keys(subquery[propId]).forEach(function(opId) {
                    if (["$in", "$nin"].includes(opId)) {
                        ...
                        query[propId][opId] = query[propId][opId].concat(subquery[propId][opId]);
                    }
                    else {
    ...
    var importSubquery = function(query, conjunction) {
        ...
        Object.keys(query).forEach(function(key) {
            if (key === "$and") {
                rawRows = rawRows.concat(_.flatten(query[key].map(function(andBranch) {
                    return importSubquery(andBranch, countlyQueryBuilder.RowConj.AND);
                })));
                return;
            }
            if (key === "$or") {
                rawRows = rawRows.concat(_.flatten(query[key].map(function(orBranch, idx) {
                    if (idx === 0) {
                        return importSubquery(orBranch, countlyQueryBuilder.RowConj.AND);
                    }
                    return importSubquery(orBranch, countlyQueryBuilder.RowConj.OR);
                })));
                return;
            }
            var field = query[key];
            if (Object.prototype.hasOwnProperty.call(field, "$gte") &&
            ...
            Object.keys(field).forEach(function(mongoOp) {
                var op = {
                    ...
                    "$exists": function(data) {
                        ...
                        return {
                            ...
                            valueData: [key.slice(dotIndex + 1, key.length)]
                        };
                    },
                    "$regex": function(data) {
                        return {
                ...
                if (op) {
                    ...
                    if (mongoOp === "$exists") {
                        var rowToPush = _.extend(baseObj, op(field[mongoOp]));
                        var rowIndex = rawRows.findIndex(function(row) {
                            return row.propertyId === rowToPush.propertyId;
                        });
                        ...
                        if (rowIndex === -1) {
                            ...
                            rawRows[rowIndex].valueData.push(rowToPush.valueData[0]);
                        }
                    }
                    else if (typeof op === "function") {
                        rawRows.push(_.extend(baseObj, op(field[mongoOp])));
                    }
                    else {
                        rawRows.push(_.extend(baseObj, {
                            operatorId: op,
    ...
    countlyQueryBuilder.registerQueryFormat("crashgroups", {
        import: function(value) {
            ...
            return new countlyQueryBuilder.FrozenQuery({
                rows: importSubquery(value.query).map(function(row, idx) {
                    row.id = idx;
        ...
        export: function(currentQuery) {
            ...
            Object.keys(grouped).forEach(function(key) {
                if (key === countlyQueryBuilder.NO_GROUP) {
                    mainObject = exportOrGroup(grouped[countlyQueryBuilder.NO_GROUP]);
                }
                else {
            ...
            if (exportedOrGroups.length === 0) {
                return { query: mainObject || {}, byVal: currentQuery.groupByProps };
            }
            else if (exportedOrGroups.length === 1) {
                return { query: _.extend(mainObject || {}, exportedOrGroups[0]), byVal: currentQuery.groupByProps };
            }
            else {
                if (mainObject) {
