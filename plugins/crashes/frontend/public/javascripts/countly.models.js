...
(function(countlyCrashes) {
    ...
    countlyCrashes.getVuexModule = function() {
        ...
        _overviewSubmodule.getters.dashboardData = function(state) {
            ...
            if ("data" in state.rawData) {
                dashboard = countlyCommon.getDashboardData(
                    ...
                    ["cru", "crau", "cruf", "crunf", "crauf", "craunf", "cr_u"],
                    null,
                    countlyCrashes.clearObject
                );
            }
            else {
                return dashboard;
            ...
            function populateMetric(metric, isPercent) {
                if (
                    dashboard[metric].total !== 0 &&
                    dashboard[metric]["prev-total"] !== 0
                ) {
                    if (isPercent) {
                        dashboard[metric].change =
                            (
                                dashboard[metric].total -
                                dashboard[metric]["prev-total"]
                            ).toFixed(1) + "%";
                    }
                    else {
                        dashboard[metric].change =
                ...
                if (
                    dashboard[metric].total === dashboard[metric]["prev-total"]
                ) {
                ...
                if (metric in derivations) {
                    dashboard[metric].isEstimate = derivations[metric].some(
                        function(targetName) {
                ...
                if (
                    ...
                    ["total", "prev-total"].forEach(function(prop) {
                        dashboard[metric][prop] =
                            dashboard[metric][prop].toFixed(2) + "%";
                    });
                }
            }
            ...
            Object.keys(derivations).forEach(function(name) {
                ["total", "prev-total"].forEach(function(prop) {
                    dashboard[name][prop] = derivations[name].reduce(function(
                        acc,
                        targetName
                    ) {
            ...
            ["cr-session", "crtf", "crtnf", "crau", "crses"].forEach(function(
                metric
            ) {
                dashboard[metric] = {};
            });
            ...
            ["total", "prev-total"].forEach(function(prop) {
                dashboard["cr-session"][prop] =
                    dashboard.cr_s[prop] === 0
                        ? 0
                        : Math.round(
                            Math.min(
                                dashboard.cr[prop] / dashboard.cr_s[prop],
                                1
                            ) * 100
                        ) / 100;
                dashboard.crau[prop] = Math.max(
                    0,
                    dashboard.crauf[prop] + dashboard.craunf[prop]
                );
                dashboard.crses[prop] = Math.max(
                    0,
                    dashboard.crfses[prop] + dashboard.crnfses[prop]
                );
                dashboard.crtf[prop] =
                    dashboard.cr_s[prop] === 0
                        ? 0
                        : Math.round(
                            Math.min(
                                dashboard.crf[prop] / dashboard.cr_s[prop],
                                1
                            ) * 100
                        ) / 100;
                dashboard.crtnf[prop] =
                    dashboard.cr_s[prop] === 0
                        ? 0
                        : Math.round(
                            Math.min(
                                dashboard.crnf[prop] / dashboard.cr_s[prop],
                                1
                            ) * 100
                        ) / 100;
            });
            ...
            ["crau", "craunf", "crauf"].forEach(function(name) {
                ["total", "prev-total"].forEach(function(prop) {
                    dashboard[name][prop] = Math.min(
                        100,
                        dashboard.cr_u[prop] === 0 ||
                            dashboard[name][prop] === 0
                            ? 100
                            : ((dashboard[name][prop] - dashboard.cr_u[prop]) /
                                  dashboard.cr_u[prop]) *
                                  100
                    );
                });
                ...
                ["total", "prev-total"].forEach(function(prop) {
                    dashboard[name][prop] = Math.min(
                        ...
                        dashboard.cr_s[prop] === 0 ||
                            dashboard[name][prop] === 0
                            ? 100
                            : ((dashboard[name][prop] - dashboard.cr_s[prop]) /
                                  dashboard.cr_s[prop]) *
                                  100
                    );
                });
        ...
        _overviewSubmodule.getters.chartData = function(state) {
            ...
            return function(metric, name) {
                ...
                var metricChartConfig = {
                    "cr-session": {
                        labelKey: "crashes.total-per-session",
                        colorIndex: 1,
                    },
                ...
                if (typeof metricChartConfig !== "undefined") {
                    chartData = [
                        {
                            data: [],
                            label: jQuery.i18n.map[metricChartConfig.labelKey],
                            color: "#52a3ef",
                            mode: "ghost",
                        },
                        {
                ...
                name =
                    ...
                    state.activeFilter.fatality !== "both"
                        ? metricNames[metric][state.activeFilter.fatality]
                        : metric);
                ...
                var metricDataProcessors = {
                    "^cr-session$": function(obj) {
                        return obj.cr_s === 0
                            ? 0
                            : Math.round(
                                Math.min((obj.crf + obj.crnf) / obj.cr_s, 1) *
                                      100
                            ) / 100;
                    },
                    "^crses$": function(obj) {
                        return obj.cr_s === 0 || obj.crfses + obj.crnfses === 0
                            ? 100
                            : Math.round(
                                Math.min(
                                    Math.max(
                    ...
                    "^crau$": function(obj) {
                        return obj.cr_u === 0 || obj.crauf + obj.craunf === 0
                            ? 100
                            : Math.round(
                                Math.min(
                                    Math.max(
                                        (obj.crauf + obj.craunf - obj.cr_u) /
                                              obj.cr_u,
                                        0
                                    ),
                                    1
                    ...
                    "^crn?fses$": function(obj) {
                        return obj.cr_s === 0 || obj[name] === 0
                            ? 100
                            : Math.round(
                                Math.min(obj[name] / obj.cr_s, 1) * 10000
                            ) / 100;
                    },
                    "^craun?f$": function(obj) {
                        return obj.cr_s === 0 || obj[name] === 0
                            ? 100
                            : Math.round(
                                Math.min(obj[name] / obj.cr_u, 1) * 10000
                            ) / 100;
                    },
                };
                ...
                var processorKey = Object.keys(metricDataProcessors).find(
                    function(keyRegex) {
                ...
                var results = countlyCommon.extractChartData(
                    state.filteredData.data,
                    countlyCrashes.clearObject,
                    chartData,
                    dataProps
                ...
                dataProps.forEach(function(dp, index) {
                    ...
                    seriesData.name =
                        dp.period === "previous"
                            ? "previous " + results.chartDP[index].label
                            : results.chartDP[index].label;
        ...
        _overviewSubmodule.getters.statistics = function(state) {
                ...
                statistics.users = {
                    affected: {
                        ...
                        totalPercent: parseFloat(
                            (
                                (state.rawData.users.total === 0
                                    ? 0
                                    : state.rawData.users.affected /
                                      state.rawData.users.total) * 100
                            ).toFixed(2)
                        ),
                        fatal: state.rawData.users.fatal,
                        fatalPercent: parseFloat(
                            (
                                (state.rawData.users.total === 0
                                    ? 0
                                    : state.rawData.users.fatal /
                                      state.rawData.users.total) * 100
                            ).toFixed(2)
                        ),
                        nonFatal:
            ...
            if ("crashes" in state.rawData) {
                statistics.crashes = {
                    ...
                    fatalPercent:
                        (state.rawData.crashes.total === 0
                            ? 0
                            : state.rawData.crashes.fatal /
                              state.rawData.crashes.total) * 100,
                    nonFatal: state.rawData.crashes.nonfatal,
                    nonFatalPercent:
                        (state.rawData.crashes.total === 0
                            ? 0
                            : state.rawData.crashes.nonfatal /
                              state.rawData.crashes.total) * 100,
                    new: state.rawData.crashes.news,
                    newPercent:
                        (state.rawData.crashes.total === 0
                            ? 0
                            : state.rawData.crashes.news /
                              state.rawData.crashes.total) * 100,
                    resolved: state.rawData.crashes.resolved,
                    resolvedPercent: parseFloat(
                        (
                            (state.rawData.crashes.total === 0
                                ? 0
                                : state.rawData.crashes.resolved /
                                  state.rawData.crashes.total) * 100
                        ).toFixed(2)
                    ),
                    unresolved:
                        state.rawData.crashes.total -
                        state.rawData.crashes.resolved,
                    unresolvedPercent: parseFloat(
                        (
                            (state.rawData.crashes.total === 0
                                ? 0
                                : (state.rawData.crashes.total -
                ...
                Object.keys(state.rawData.crashes.os).forEach(function(
                    platform
                ) {
                    statistics.topPlatforms[platform] = {
                        count: countlyCommon.getShortNumber(
                            state.rawData.crashes.os[platform]
                        ),
                        percent:
                            (statistics.crashes.total === 0
                                ? 0
                                : state.rawData.crashes.os[platform] /
                                  statistics.crashes.total) * 100,
                    };
                });

                statistics.topPlatformsOrder = Object.entries(
                    statistics.topPlatforms
                )
        ...
        _overviewSubmodule.getters.appVersions = function(state) {
            return "crashes" in state.rawData
                ? Object.keys(state.rawData.crashes.app_version)
                : [];
        };

        _overviewSubmodule.getters.platforms = function(state) {
            return "crashes" in state.rawData
                ? Object.keys(state.rawData.crashes.os)
                : [];
        };

        _overviewSubmodule.actions.setCrashgroupsFilter = function(
            context,
            value
        ) {
        ...
        _overviewSubmodule.actions.refresh = function(context) {
            ...
            Object.keys(filterMapping).forEach(function(filterKey) {
                if (context.state.activeFilter[filterKey] !== "all") {
                    filterParams[filterMapping[filterKey]] =
                        context.state.activeFilter[filterKey];
                }
            });
        ...
        _overviewSubmodule.actions.setSelectedAsResolved = function(
            context,
            selectedIds
        ) {
        ...
        _overviewSubmodule.actions.setSelectedAsResolving = function(
            context,
            selectedIds
        ) {
            return countlyCrashes.manipulateCrashgroup(
                selectedIds,
                "resolving"
            );
        };

        _overviewSubmodule.actions.setSelectedAsUnresolved = function(
            context,
            selectedIds
        ) {
            return countlyCrashes.manipulateCrashgroup(
        ...
        _overviewSubmodule.actions.setSelectedAsShown = function(
            context,
            selectedIds
        ) {
        ...
        _overviewSubmodule.actions.setSelectedAsHidden = function(
            context,
            selectedIds
        ) {
        ...
        _overviewSubmodule.actions.setSelectedAsDeleted = function(
            context,
            selectedIds
        ) {
        ...
        _crashgroupSubmodule.getters.commonMetrics = function(state) {
            if (typeof state.crashgroup._id !== "undefined") {
                return {
                    ...
                    crashFrequency:
                        "session" in state.crashgroup
                            ? state.crashgroup.session.total /
                              state.crashgroup.session.count
                            : 0,
        ...
        _crashgroupSubmodule.getters.mobileDiagnostics = function(state) {
            if (typeof state.crashgroup._id !== "undefined") {
                ...
                Object.keys(diagnostics).forEach(function(diagnosticKey) {
                    ...
                    if (
                        typeof diagnostics[diagnosticKey].total === "undefined"
                    ) {
                        diagnostics[diagnosticKey].total = 0;
                    }
                    if (
                        typeof diagnostics[diagnosticKey].count === "undefined"
                    ) {
                        diagnostics[diagnosticKey].count = 0;
                    }
                    if (diagnostics[diagnosticKey].count > 0) {
                        diagnostics[diagnosticKey].average =
                            diagnostics[diagnosticKey].total /
                            diagnostics[diagnosticKey].count;
                    }
                    else {
                        diagnostics[diagnosticKey].average = 0;
                    }
                    diagnostics[diagnosticKey].tooltip =
                        tooltips[diagnosticKey];
                });
        ...
        _crashgroupSubmodule.getters.mobileMetrics = function(state) {
            if (
                countlyGlobal.apps[countlyCommon.ACTIVE_APP_ID].type !==
                    "web" &&
                typeof state.crashgroup._id !== "undefined" &&
                "root" in state.crashgroup
            ) {
                return {
                    rootedPercent:
                        (state.crashgroup.root.yes / state.crashgroup.reports) *
                        100,
                    onlinePercent:
                        (state.crashgroup.online.yes /
                            state.crashgroup.reports) *
                        100,
                    mutedPercent:
                        (state.crashgroup.muted.yes /
                            state.crashgroup.reports) *
                        100,
                    backgroundPercent:
                        (state.crashgroup.background.yes /
                            state.crashgroup.reports) *
                        100,
                };
            }
        ...
        _crashgroupSubmodule.getters.chartByOptions = function(state) {
            ...
            if ("custom" in state.crashgroup) {
                Object.keys(state.crashgroup.custom).forEach(function(
                    customKey
                ) {
                    chartByOptions.push({
                    ...
                    series: [
                        {
                            ...
                            color: "#F96300",
                            name:
                                CV.i18n("crashes.fatal_crash_count") +
                                " by " +
                                chartBy,
                        },
                    ],
        ...
        _crashgroupSubmodule.getters.crashEventLog = function(state) {
            return function(crashId) {
                var crashIndex = state.crashgroup.data.findIndex(function(
                    crash
                ) {
                    return crash._id === crashId;
                });
                return countlyCrashesEventLogs.formatEventLog(
                    state.crashgroup.data[crashIndex].eventlog
                );
            };
        };

        _crashgroupSubmodule.getters.appVersions = function(state) {
            return "data" in state.crashgroup
                ? Object.keys(state.crashgroup.app_version).map(function(
                    version
                ) {
        ...
        _crashgroupSubmodule.getters.platforms = function(state) {
            return "data" in state.crashgroup
                ? Object.keys(state.crashgroup.os_version).map(function(
                    version
                ) {
                    return (
                        state.crashgroup.os +
                ...
                countlyVue.$.ajax({
                    ...
                    success: function(crashgroupJson) {
                        if (
                            ...
                            if (
                                crashgroupJson.data.length &&
                                crashgroupJson.os === "iOS"
                            ) {
                                crashgroupJson.binary_images =
                            ...
                            if (typeof countlyCrashSymbols !== "undefined") {
                                var crashes = [
                                    {
                                        ...
                                        app_version:
                                            crashgroupJson.latest_version,
                                        build_uuid:
                                            crashgroupJson.data &&
                                            crashgroupJson.data.length &&
                                            crashgroupJson.data[0].build_uuid,
                                    },
                                ];
                                ...
                                ajaxPromise.then(function(symbolIndexing) {
                                    crashes.forEach(function(
                                        ...
                                        var symbol_id =
                                            countlyCrashSymbols.canSymbolicate(
                                                crash,
                                                symbolIndexing.symbolIndexing ||
                                                    symbolIndexing
                                            );
                ...
                countlyVue.$.ajax({
                    ...
                    success: function(json) {
                        Object.keys(json).forEach(function(crashId) {
                            ...
                            context.state.crashgroup.data[crashIndex].eventlog =
                                json[crashId];
                        });
                        resolve(json);
                    },
        ...
        _crashgroupSubmodule.actions.markResolved = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "resolve")
                .then(function() {
                    context.state.crashgroup.is_resolved = true;
                    context.state.crashgroup.resolved_version =
                        context.state.crashgroup.latest_version;
                });
        };

        _crashgroupSubmodule.actions.markResolving = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "resolving")
                .then(function() {
        ...
        _crashgroupSubmodule.actions.markUnresolved = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "unresolve")
                .then(function() {
                    context.state.crashgroup.is_resolved = false;
                });
        };
        ...
        _crashgroupSubmodule.actions.markSeen = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "view")
                .then(function() {
        ...
        _crashgroupSubmodule.actions.show = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "show")
                .then(function() {
        ...
        _crashgroupSubmodule.actions.hide = function(context) {
            return countlyCrashes
                .manipulateCrashgroup(context.state.crashgroup._id, "hide")
                .then(function() {
        ...
        _crashgroupSubmodule.actions.addComment = function(context, body) {
            var originalPromise = countlyCrashes.manipulateCrashgroup(
                context.state.crashgroup._id,
                "add_comment",
                { text: body }
            );
        ...
        _crashgroupSubmodule.actions.deleteComment = function(
            context,
            commentId
        ) {
            return countlyCrashes.manipulateCrashgroup(
        ...
        var crashgroupsResource = countlyVue.vuex.ServerDataTable(
            "crashgroups",
            {
                columns: [
                    "name",
    ...
    countlyCrashes.generateBadges = function(crash) {
        ...
        if (crash.nonfatal) {
            badges.push({
                type: "neutral",
                content: CV.i18n("crashes.nonfatal"),
            });
        }
        else {
            badges.push({
                type: "negative",
                content: CV.i18n("crashes.fatal"),
            });
        }
        ...
        if (crash.is_resolved) {
            badges.push({
                type: "positive",
                content:
                    CV.i18n("crashes.resolved") +
                    " (" +
                    crash.resolved_version +
                    ")",
            });
        }
            ...
            badges.push({
                type: "neutral",
                content: CV.i18n("crashes.resolving"),
            });
        }
        else {
            badges.push({
                type: "neutral",
                content: CV.i18n("crashes.unresolved"),
            });
        }
        ...
        if (crash.is_renewed) {
            badges.push({
                type: "neutral",
                content: CV.i18n("crashes.reoccuring"),
            });
        }

        if ("session" in crash) {
            var frequency = Math.round(
                crash.session.total / crash.session.count
            );
            if (frequency === 1) {
                badges.push({
                    type: "info",
                    content: CV.i18n("crashes.every-session"),
                });
            }
            else if (frequency > 1) {
                badges.push({
                    type: "info",
                    content: CV.i18n("crashes.every-n-sessions", frequency),
                });
            }
        }
    ...
    if (
        ...
        countlyAuth.validateRead(FEATURE_NAME)
    ) {
