...
countlyCrashes.getVuexModule = function() {
    ...
    _overviewSubmodule.getters.chartData = function(state) {
        ...
        return function(metric, name) {
            ...
            if (typeof metricChartConfig !== "undefined") {
                chartData = [
                    {data: [], label: i18n(metricChartConfig.labelKey), color: "#52a3ef", mode: "ghost" },
                    {data: [], label: i18n(metricChartConfig.labelKey), color: countlyCommon.GRAPH_COLORS[metricChartConfig.colorIndex]}
                ];
            }
            else {
                chartData = [
    ...
    _crashgroupSubmodule.getters.crashes = function(state) {
        if ("data" in state.crashgroup) {
            return state.crashgroup.data.map(function(item) {
                var transformedVersion = transformAppVersion(item.app_version);
                item.app_version_for_sort = transformedVersion;
                return item;
            });
        }
        else {
    ...
    _crashgroupSubmodule.getters.mobileDiagnostics = function(state) {
        if (typeof state.crashgroup._id !== "undefined") {
            ...
            var tooltips = {
                ram: i18n("crashes.help-ram"),
                disk: i18n("crashes.help-disk"),
                battery: i18n("crashes.help-battery"),
                running: i18n("crashes.help-run"),
                sessions: i18n("crashes.help-session")
            };
            ...
            diagnostics.running.unit = i18n("crashes.minutes-short");
    ...
    _crashgroupSubmodule.getters.chartData = function(state) {
        return function(chartBy) {
            ...
            var chartOptions = {
                ...
                series: [
                    {data: Object.values(mapObj), color: "#F96300", name: i18n("crashes.fatal_crash_count") + ' by ' + chartBy}
                ]
            };
    ...
    _crashgroupSubmodule.getters.crashEventLog = function(state) {
        return function(cid) {
            var crashIndex = state.crashgroup.data.findIndex(function(crash) {
                return crash._id === cid;
            });
            return window.countlyCrashesEventLogs.formatEventLog(state.crashgroup.data[crashIndex].eventlog);
        };
    };
    ...
    _crashgroupSubmodule.actions.initialize = function(context, gid) {
        context.state.isLoading = true;
        context.state.crashgroup._id = gid;
    ...
    _crashgroupSubmodule.actions.generateEventLogs = function(context, crashIds) {
        return new Promise(function(resolve, reject) {
            ajax({
                ...
                dataType: "json",
                success: function(json) {
                    Object.keys(json).forEach(function(cid) {
                        ...
                        context.state.crashgroup.data[crashIndex].eventlog = json[cid];
                    });
                    resolve(json);
                },
                error: reject
    ...
    _crashSubmodule.actions.initialize = function(context, cid) {
        context.state.crash._id = cid;
        return context.dispatch("refresh");
    };
    ...
    var crashgroupsResource = ServerDataTable("crashgroups", {
    ...
    var _module = {
        ...
        mutations: {},
        submodules: [
            Module("overview", _overviewSubmodule),
            Module("crashgroup", _crashgroupSubmodule),
            Module("crash", _crashSubmodule),
            crashgroupsResource
        ]
    };

    return Module("countlyCrashes", _module);
};
...
countlyCrashes.generateBadges = function(crash) {
    ...
    if (crash.nonfatal) {
        badges.push({type: "neutral", content: i18n("crashes.nonfatal")});
    }
    else {
        badges.push({type: "negative", content: i18n("crashes.fatal")});
    }

    if (crash.is_resolved) {
        badges.push({type: "positive", content: i18n("crashes.resolved") + " (" + crash.resolved_version + ")"});
    }
    else if (crash.is_resolving) {
        badges.push({type: "neutral", content: i18n("crashes.resolving")});
    }
    else {
        badges.push({type: "neutral", content: i18n("crashes.unresolved")});
    }

    if (crash.is_renewed) {
        badges.push({type: "neutral", content: i18n("crashes.reoccuring")});
    }
    ...
    if ("session" in crash) {
        ...
        if (frequency === 1) {
            badges.push({type: "info", content: i18n("crashes.every-session")});
        }
        else if (frequency > 1) {
            badges.push({type: "info", content: i18n("crashes.every-n-sessions", frequency)});
        }
    }

    if (crash.is_new) {
        badges.push({type: "info", content: i18n("crashes.new")});
    }
...
if (countlyCommon.ACTIVE_APP_ID !== 0 && validateRead(FEATURE_NAME)) {
    countlyCrashes.loadList(countlyCommon.ACTIVE_APP_ID);
}

app.addAppSwitchCallback(function(appId) {
    if (app._isFirstLoad !== true && validateRead(FEATURE_NAME)) {
...
// Expose globally for non-ESM plugins that reference window.countlyCrashes
window.countlyCrashes = countlyCrashes;

export default countlyCrashes;
