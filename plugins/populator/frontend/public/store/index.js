...
function User() {
    ...
    this.getFeedbackEvents = function() {
        ...
        if (countlyGlobal.plugins.indexOf("surveys") !== -1 && validateCreate("surveys")) {
...
function generateWidgets(done) {
    ...
    function generateRatingWidgets(callback) {
        if (countlyGlobal.plugins.indexOf("star-rating") !== -1 && validateCreate("star-rating") && _featuresToPopulate.includes("star-rating")) {
    ...
    function generateNPSWidgets(callback) {
        if (countlyGlobal.plugins.indexOf("surveys") !== -1 && validateCreate("surveys") && _featuresToPopulate.includes("surveys")) {
...
function generateCampaigns(callback) {
    if (!isPluginEnabled("attribution") || typeof window.countlyAttribution === "undefined") {
        callback();
        return;
    }
    ...
    async function getUsers() {
        ...
        async function createAndProcessUsers() {
            ...
            if (environment.length < template.uniqueUserCount) { // if environment users are less than template user count(in case of stop generate or error), complete the rest with new users in next run
                notify({type: 'warning', message: i18n("populator.warning-environment-users", environment.length, template.uniqueUserCount, template.uniqueUserCount - environment.length), sticky: true});
    ...
    if (environment && environment.length) {
        ...
        generateWidgets(function() {
            generateCampaigns(async function() {
                ...
                if (countlyGlobal.plugins.indexOf("ab-testing") !== -1 && validateCreate("ab-testing") && _featuresToPopulate.includes("ab-testing")) {
...
countlyPopulator.ensureJobs = function() {
    ...
    if (isPluginEnabled("cohorts") && typeof window.countlyCohorts !== "undefined" && validateCreate('cohorts') && _featuresToPopulate.includes("cohorts")) {
    ...
    if (isPluginEnabled("funnels") && typeof window.countlyFunnel !== "undefined" && validateCreate('funnels') && _featuresToPopulate.includes("funnels")) {
        ...
        window.countlyFunnel.createFunnel({
    ...
    if (countlyGlobal.plugins.indexOf('crash_symbolication') !== -1 && validateCreate('crash_symbolication') && _featuresToPopulate.includes("crashes")) {
    ...
    if (countlyGlobal.plugins.indexOf('drill') !== -1) {
        jQuery.ajax({
            ...
            error: function() {
                notify({message: i18n("populator.failed-message-regenerate-meta"), type: "error"});
            }
        });
    }

    if (isPluginEnabled("push") && _featuresToPopulate.includes("push")) {
...
countlyPopulator.getTemplate = function(templateId, callback) {
    ...
    if (typeof foundDefault !== "undefined") {
        ...
        jQuery.ajax({
            ...
            success: callback,
            error: function() {
                notify({message: i18n("populator.failed-to-fetch-template", templateId), type: "error"});
            }
        });
    }
};
...
countlyPopulator.getTemplates = function(platformType, callback) {
    ...
    jQuery.ajax({
        ...
        error: function() {
            notify({message: i18n("populator.failed-to-fetch-templates"), type: "error"});
        }
    });
};
...
countlyPopulator.removeTemplate = function(templateId, callback) {
    ...
    if (typeof foundDefault !== "undefined") {
        ...
        jQuery.ajax({
            ...
            success: callback,
            error: function() {
                notify({message: i18n("populator.failed-to-remove-template", templateId), type: "error"});
            }
        });
    }
};
...
countlyPopulator.checkEnvironment = function(environmentName, callback) {
    ...
    jQuery.ajax({
        ...
        error: function() {
            notify({message: i18n("populator.failed-to-fetch-environment"), type: "error"});
        }
    });
};
...
countlyPopulator.getEnvironments = function(callback) {
    ...
    jQuery.ajax({
        ...
        error: function() {
            notify({message: i18n("populator.failed-to-fetch-environments"), type: "error"});
        }
    });
};
...
countlyPopulator.removeEnvironment = function(templateId, environmentId, callback) {
    ...
    jQuery.ajax({
        ...
        error: function() {
            notify({message: i18n("populator.failed-to-delete-environment"), type: "error"});
        }
    });
};
...
window.countlyPopulator = countlyPopulator;

export default countlyPopulator;
