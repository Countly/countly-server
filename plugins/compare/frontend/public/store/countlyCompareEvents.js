...
countlyCompareEvents.helpers = {
    ...
    getTableRows: function(context) {
        ...
        var selectedEvents = context.state.selectedEvents;
        var allEventsData = context.state.allEventsData;
        var eventsData = context.state.allEventsCompareData;
        for (var i = 0; i < selectedEvents.length; i++) {
            ...
            var data = countlyCommon.extractChartData(eventsData[countlyCompareEvents.helpers.encode(selectedEvents[i])], countlyCompareEvents.helpers.clearObject, chartData, propData);
            var totals = data.chartData.reduce(function(acc, curr) {
                for (var key in acc) {
                    acc[key] += curr[key] || 0;
                }
                return acc;
    ...
    getLineChartData: function(context, selectedEvents) {
        var series = [];
        var metric = context.state.selectedGraphMetric;
        if (selectedEvents.length === 1) {
            ...
            var prevObj = {
                name: i18n("compare.events.previous.period"),
                data: prevData,
            };
    ...
    getLegendData: function(selectedEvents, groupData, map) {
        ...
        if (selectedEvents.length === 1) {
            ...
            prevObj.name = i18n("compare.events.previous.period");
    ...
    getAllEventsList: function(eventsList, groupList) {
        ...
        if (groupList) {
            groupList.forEach(function(item) {
                if (item.status) {
                    var obj = {
                        "label": countlyCompareEvents.helpers.decode(item.name) + "(" + i18n("events.all.group") + ")",
                        "value": item._id
                    };
                    allEvents.push(obj);
                }
