...
createMetricModel(countlyViews, {name: "views"}, jQuery);
...
var editTableResource = ServerDataTable("viewsEditTable", {
    ...
    onError: function(context, error) {
        if (error) {
            if (error.status !== 0) { //not on canceled ones
                window.app.activeView.onError(error);
            }
        }
    }
});

var viewsTableResource = ServerDataTable("viewsMainTable", {
    ...
    onRequest: function(context) {
        ...
        var selectedKey = selectedInfo.selectedSegment || "";
        var selectedValue = selectedInfo.selectedSegmentValue || "";
    ...
    if (!countlyCommon.DEBUG) {
        ...
        return $.when(
            ...
            function() {
                ...
                if (countlyCommon.periodObj.numberOfDays === 1) {
                    bucket = "h";
                }
                return $.when($.ajax({
                    type: "GET",
...
if (countlyGlobal.member && countlyGlobal.member.api_key && countlyCommon.ACTIVE_APP_ID !== 0 && validateRead(FEATURE_NAME) && isPluginEnabled(FEATURE_NAME)) {
...
//currently this function is used from other parts in countly. Keep wrapper.
...
countlyViews.refresh = function() {
    if (!countlyCommon.DEBUG) {
        ...
        var bucket = "d";
        if (countlyCommon.periodObj._period === "month") {
            bucket = "m";
        }
        if (countlyCommon.periodObj.numberOfDays === 1) {
            bucket = "h";
...
window.countlyViews = countlyViews;


// --- countlyViewsPerSession ---

var countlyViewsPerSession = {};
...
countlyViewsPerSession.service = {
    mapViewsPerSessionSeries: function(dto) {
        ...
        return [{data: viewsPerSessionSerieData, label: i18n("views-per-session.title")}];
    },
    ...
    fetchViewsPerSession: function() {
        var self = this;
        return new Promise(function(resolve, reject) {
            window.countlySession.initialize()
                .then(function() {
                    var viewsPerSessionData = window.countlySession.getRangeData("vc", "v-ranges", countlyViewsPerSession.helpers.explainViewsPerSessionRange, countlyViewsPerSession.helpers.getViewsPerSessionRange());
...
countlyViewsPerSession.getVuexModule = function() {
    ...
    return Module("countlyViewsPerSession", {
        state: getInitialState,
...
window.countlyViewsPerSession = countlyViewsPerSession;

export { countlyViews, countlyViewsPerSession };
export var getVuexModule = countlyViews.getVuexModule;
export var getViewsPerSessionVuexModule = countlyViewsPerSession.getVuexModule;
