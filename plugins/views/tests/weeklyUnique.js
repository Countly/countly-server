...
describe('Testing views weekly-unique counting', function() {
    describe('Same user viewing the same view in two sessions within one ISO week', function() {
        ...
        it('records the first session view', function(done) {
            request
                .get('/i?begin_session=1&app_key=' + APP_KEY + '&device_id=' + DEVICE_ID + '&timestamp=' + t1 + '&events=' + encodeURIComponent(viewEvent))
                .expect(200)
                .end(function(err) {
                    if (err) {
                        return done(err);
        ...
        it('records the second session view in the same week', function(done) {
            request
                .get('/i?begin_session=1&app_key=' + APP_KEY + '&device_id=' + DEVICE_ID + '&timestamp=' + t2 + '&events=' + encodeURIComponent(viewEvent))
                .expect(200)
                .end(function(err) {
                    if (err) {
                        return done(err);
