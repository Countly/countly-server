...
-- @table: drill_events_raw
-- @engine: MergeTree
-- @sharding: sipHash64(a, uid)
--
-- Stores the original (pre-obfuscation) version of drill events where at least one
-- PII rule matched and mutated the event. Written by a second ClickHouse Kafka
-- Connect sink connector that consumes the same drill-events topic and uses the
-- RawEventExtractor SMT to reconstruct the original event from the `raw` delta field.
-- Events without a `raw` field (no PII match) are filtered out by the SMT.
