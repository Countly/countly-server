...
app.get(countlyConfig.path+'/setup', function (req, res, next) {
    countlyDb.collection('members').count({}, function (err, memberCount) {
        if (memberCount) {
            ...
            res.render('setup', {countlyTitle:req.countly.title, countlyPage:req.countly.page, "csrf":req.csrfToken(), path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || ""});
        }
    });
});
    ...
    if (req.session.uid) {
        ...
        countlyDb.collection('members').count({}, function (err, memberCount) {
            if (memberCount) {
                ...
                res.render('login', { countlyTitle:req.countly.title, countlyPage:req.countly.page, "message":req.flash('info'), "csrf":req.csrfToken(), path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || "" });
            } else {
...
app.get(countlyConfig.path+'/forgot', function (req, res, next) {
    if (req.session.uid) {
        ...
        res.render('forgot', { countlyTitle:req.countly.title, countlyPage:req.countly.page, "csrf":req.csrfToken(), "message":req.flash('info'), path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || ""});
    }
});
        ...
        countlyDb.collection('password_reset').findOne({prid:req.params.prid}, function (err, passwordReset) {
            ...
            if (passwordReset && !err) {
                if (timestamp > (passwordReset.timestamp + 600)) {
                    ...
                    res.render('reset', { countlyTitle:req.countly.title, countlyPage:req.countly.page, "csrf":req.csrfToken(), "prid":req.params.prid, "message":"", path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || "" });
                }
            } else {
                req.flash('info', 'reset.invalid');
                res.redirect(countlyConfig.path+'/forgot');
...
app.post(countlyConfig.path+'/reset', function (req, res, next) {
    if (req.body.password && req.body.again && req.body.prid) {
        ...
        res.render('reset', { countlyTitle:req.countly.title, countlyPage:req.countly.page, "csrf":req.csrfToken(), "prid":req.body.prid, "message":"", path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || "" });
    }
});
        ...
        countlyDb.collection('members').findOne({"email":req.body.email}, function (err, member) {
            if (member) {
                ...
                countlyDb.collection('password_reset').insert({"prid":prid, "user_id":member._id, "timestamp":timestamp}, {safe:true}, function (err, password_reset) {
                    ...
                    res.render('forgot', { countlyTitle:req.countly.title, countlyPage:req.countly.page, "message":"forgot.result", "csrf":req.csrfToken(), path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || "" });
                });
            } else {
                res.render('forgot', { countlyTitle:req.countly.title, countlyPage:req.countly.page,"message":"forgot.result", "csrf":req.csrfToken(), path:countlyConfig.path || "", cdn:countlyConfig.cdn || "", themeFiles:req.themeFiles, javascript:req.template.js || "", form:req.template.form || "", html:req.template.html || "" });
            }
