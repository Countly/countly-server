import { ajax } from '../../../javascripts/countly/vue/core.js';
...
countlyUserManagement.resetFailedLogins = function(id, callback) {
    ...
    Promise.all([resetLoginsWithEmail, resetLoginsWithUsername])
        .then(function() {
            callback();
        })
        .catch(function(err) {
            callback(err && err.responseJSON ? err.responseJSON.result : err);
        });
};
...
export default countlyUserManagement;
