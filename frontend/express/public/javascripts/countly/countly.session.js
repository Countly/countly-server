...
CountlyHelpers.createMetricModel(countlySession, { name: "users", estOverrideMetric: "users" }, jQuery);
...
countlySession.getSessionData = function(sessionModel) {
    ...
    const map = { t: "total-sessions", n: "new-users", u: "total-users", d: "total-duration", e: "events", p: "paying-users", m: "messaging-users" };
    const ret = {};
    const data = countlyCommon.getDashboardData(sessionModel, ["t", "n", "u", "d", "e", "p", "m"], ["u", "p", "m"], { u: "users" }, countlySession.clearObject);
    ...
    const changeReturning = countlyCommon.getPercentChange(
    ...
    const sparkLines = countlyCommon.getSparklineData(sessionModel, {
        "total-sessions": "t",
        "new-users": "n",
        "total-users": "u",
        "total-duration": "d",
...
function durationRange() {
    const sec = jQuery.i18n.map["common.seconds"];
    const min = jQuery.i18n.map["common.minutes"];
    const hr = jQuery.i18n.map["common.hour"];
...
function frequencyRange() {
    const localHours = jQuery.i18n.map["user-loyalty.range.hours"];
    const localDay = jQuery.i18n.map["user-loyalty.range.day"];
    const localDays = jQuery.i18n.map["user-loyalty.range.days"];
...
countlySession.getTopUserBars = function() {
    ...
    const _db = countlySession.getDb();
    const dashboardData = countlyCommon.getDashboardData(_db, ["t", "n", "u", "d", "e", "p", "m"], ["u", "p", "m"], { u: "users" }, countlySession.clearObject);
    const totalUserData = countlyCommon.extractChartData(_db, countlySession.clearObject, chartData, dataProps);
    const topUsers = _.sortBy(_.reject(totalUserData.chartData, function(obj) {
    ...
    const totalUsers = (dashboardData && dashboardData.u && dashboardData.u.total);

    for (let i = 0; i < maxItems; i++) {
        const percent = Math.floor((topUsers[i].t / totalUsers) * 100);
