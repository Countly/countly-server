...
Returns session frequency bucket distribution for the selected period.
...
```json
[
  {"f": "1", "t": 280, "percent": "56.3"},
  {"f": "2", "t": 174, "percent": "35.0"},
  {"f": "0", "t": 43, "percent": "8.7"}
]
```
...
|---|---|---|
| `[]` | Array | Frequency bucket rows sorted by count descending. |
| `[] .f` | String | Frequency bucket key. |
...
- Single mode:
  - Reads `f-ranges` metadata and extracts frequency range data for requested period.
