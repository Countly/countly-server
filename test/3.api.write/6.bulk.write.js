...
describe('Bulk writing', function(){
	...
	describe('using session tests', function(){
		it('should success', function(done){
			var params = [
				{"device_id":DEVICE_ID, "app_key":APP_KEY, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 8)},
				{"device_id":DEVICE_ID, "app_key":APP_KEY, "begin_session":1, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 7)},
				{"device_id":DEVICE_ID, "app_key":APP_KEY, "begin_session":1, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 6)},
				{"device_id":DEVICE_ID+"new", "app_key":APP_KEY, "begin_session":1, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 5)},
				{"device_id":DEVICE_ID, "app_key":APP_KEY, "session_duration":30, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 4)},
				{"device_id":DEVICE_ID, "app_key":APP_KEY, "end_session":1, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 3)},
				{"device_id":DEVICE_ID, "app_key":APP_KEY, "session_duration":30, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 2)},
				{"device_id":DEVICE_ID+"A", "app_key":APP_KEY, "end_session":1, timestamp:parseInt(new Date().getTime()/1000 - 60 * 60 * 1)}
			];
	...
	describe('Verify bulk session write', function(){
		describe('Verify sessions', function(){
			it('should match sessions tests end result', function(done){
				...
				.expect(200)
				.end(function(err, res){
					testUtils.validateSessionData(err, res, done, {meta:{"countries":["Unknown"],"f-ranges":["0"],"l-ranges":["0"]}, f:{"0":3},l:{"0":3},u:3,n:3,t:3,e:6,d:60,Unknown:true});
				});
			});
		});
		...
		describe('verify dashboard', function(){
			it('should match sessions tests end result', function(done){
				...
				.expect(200)
				.end(function(err, res){
					testUtils.validateDashboard(err, res, done, {total_sessions:3,total_users:3,new_users:3,total_time:"1.0 min",avg_time:"0.3 min",avg_requests:"2.0", platforms:[],carriers:[],resolutions:[]});
				});
			});
		});
		describe('verify countries', function(){
			it('should match sessions tests end result', function(done){
				...
				.expect(200)
				.end(function(err, res){
					testUtils.validateCountries(err, res, done, {country:"Unknown",code:"unknown",t:3,u:3,u2:6,n:3});
				});
			});
		});
