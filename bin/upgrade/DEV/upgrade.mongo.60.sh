...
isAuth=$(mongosh --eval "db.getUsers()" | grep -v "connecting" | grep "auth")
...
if [ -x "$(command -v mongosh)" ]; then
    if echo "$VER" | grep -q -i "6.0" ; then
        if echo "$FEATVER" | grep -q -i "5.0" ; then
            echo "run this command to ugprade to 6.0";
...
if ! [ -z "$isAuth" ] ; then
    ...
    echo "mongosh admin --eval \"db.adminCommand( { setFeatureCompatibilityVersion: \\\"5.0\\\" } )\""
    ...
    mongosh admin --eval "db.adminCommand( { setFeatureCompatibilityVersion: \"5.0\" } )"
