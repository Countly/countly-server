...
async function flushData(count1,count2,appID,newObj,newObj2,summedZero,monthObject,msplit,force){
    ...
    if(count2 >= bufferSize || force){
        var colName = "app_viewdata"+crypto.createHash('sha1').update("platform"+appID).digest('hex');
        var bulk = countlyDb._native.collection(colName).initializeUnorderedBulkOp();
        for(var d in newObj['platform']) {
            let iid = viewsMap[d]+"";
           /*newObj['platform'][d]['s'] = "platform";
            newObj['platform'][d]['vw'] = countlyDb.ObjectID(iid);
            newObj['platform'][d]['m'] = monthObject['_id'];
            bulk.find( {'_id': iid+'_'+monthObject['_id']} ).upsert().updateOne( { $set: newObj['platform'][d] } );
            */
        ...
        for(var d in newObj2['platform']) {
            let iid = viewsMap[d]+"";
           /*newObj2['platform'][d]['s'] = "platform";
            newObj2['platform'][d]['vw'] = countlyDb.ObjectID(iid);
            newObj2['platform'][d]['m'] = monthObject['_id'];
            bulk.find({'_id': iid+'_'+monthObject['_id']+"_m"}).upsert().updateOne({ $set:  newObj2['platform'][d]} );*/
    ...
    if(flush_zero==true){
                            ...
                            for(var d in summedZero['no-segment']) {
                                let iid = viewsMap[d]+"";
                               /* summedZero['no-segment'][d]['vw']=countlyDb.ObjectID(iid);
                                summedZero['no-segment'][d]['m']=msplit[0]+":0";
                                summedZero['no-segment'][d]['s'] = 'no-segment';
                                bulk.find({'_id': iid+'_'+msplit[0]+":0"}).upsert().updateOne({ $set:  summedZero['no-segment'][d]}  );*/
                            ...
                            for(var d in summedZero['platform']) {
                                ...
                                /*summedZero['platform'][d]['vw']=countlyDb.ObjectID(iid);
                                summedZero['platform'][d]['m']=msplit[0]+":0";
                                summedZero['platform'][d]['s'] = 'platform';
                                bulk.find({'_id': iid+'_'+msplit[0]+":0"}).upsert().updateOne({ $set:  summedZero['platform'][d]}  );*/
                            ...
                            for(var d in summedZero['no-segment']) { delete summedZero['no-segment'][d];}
                            for(var d in summedZero['platform']) { delete summedZero['platform'][d];}
...
function check_and_fix_data(appID, done) {
    ...
    countlyDb.collection('app_viewdata' + appID).aggregate([{$group:{_id:false,views:{$mergeObjects: "$meta_v2.views"}}}],function(err,res) {
        ...
        countlyDb.collection('app_viewsmeta' + appID).ensureIndex({"view":1},{'unique':1},function(){
            countlyDb.collection('app_viewsmeta' + appID).insertMany(insertObj,{"ordered":false},function(err,ress) {
                console.log("View names inserted");
                countlyDb.collection('app_viewsmeta' + appID).find({}).toArray(function(err,views){
