...
if [ "$1" = "list" ]; then
    for d in "$DIR"/../../../plugins//; do
        if [ "$2" = "states" ]; then
            PLUGIN="$(basename "$d")"
            if grep -Fq ""$PLUGIN"" "$DIR/../../../plugins/plugins.json"; then
    ...
    if [ "$1" = "enable" ]; then
        node $NODE_ARGS "$DIR/plugin.js" enable "$2"
    elif [ "$1" = "disable" ]; then
        node $NODE_ARGS "$DIR/plugin.js" disable "$2"
    elif [ "$1" = "upgrade" ]; then
        node $NODE_ARGS "$DIR/plugin.js" upgrade "$2"
    elif [ "$1" = "status" ]; then
        if grep -Fq ""$2"" "$DIR/../../../plugins/plugins.json"; then
            echo "enabled"
        else
            echo "disabled"
        fi
        ...
        shift
        node $NODE_ARGS "$DIR/plugin.js" test "$@"
    elif [ "$1" = "lint" ]; then
        cd "$DIR/../../../"
